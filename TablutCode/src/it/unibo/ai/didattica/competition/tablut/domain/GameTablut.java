...
public class GameTablut implements Game {
	...
	/**
	 * This method checks an action in a state: if it is correct the state is going
	 * to be changed, if it is wrong it throws a specific exception
	 * 
	...
	private State checkCaptureBlack(State state, Action a) {
		...
		if (a.getColumnTo() < state.getBoard().length - 2
				...
				&& (state.getPawn(a.getRowTo(), a.getColumnTo() + 2).equalsPawn("B")
						|| state.getPawn(a.getRowTo(), a.getColumnTo() + 2).equalsPawn("T"))) {
			// nero-re-trono N.B. No indexOutOfBoundException perch� se il re si trovasse
			// sul bordo il giocatore bianco avrebbe gi� vinto
